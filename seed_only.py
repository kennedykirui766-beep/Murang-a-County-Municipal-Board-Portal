...
def seed_data():
    with app.app_context():
        ...
        db.session.add_all(audit_logs_data)
        ...
        db.session.add_all(system_activities_data)
        ...
        saved_reports_data = [
            Report(
                ...
                filters=json.dumps({'municipality': 'all'}),
                data=json.dumps({'total_users': 12, 'total_meetings': 3}),
                format='json',
                municipality='all'
            )
        ]
        db.session.add_all(saved_reports_data)
